    ...
    class ConvCommon
    {
        ...
        static public void ResolveAlias(TypeInfo type, TypeDesc typeDesc, out TypeInfo realType, out TypeAttr realAttr)
        {
            ...
            else
            {
                ...
                if (refAttr.typekind != TYPEKIND.TKIND_ALIAS || (VarEnum)refAttr.tdescAlias.vt != VarEnum.VT_USERDEFINED)
                {
                    // Resolved
                    realType = refType;
                    realAttr = refAttr;
        ...
        static private bool IsRetVal(ConverterInfo info, ParamDesc paramDesc, InterfaceMemberInfo memInfo)
        {
            if (paramDesc.IsRetval)
            {
                ...
                {
                    if (memInfo.RefFuncDesc.funckind == FUNCKIND.FUNC_DISPATCH && !info.TransformDispRetVal)
                        return false;
                    return true;
                }
            }
            ...
            else
            {
                for (int i = 0; i < func.cParams; ++i)
                {
                    ...
                    if (IsRetVal(info.ConverterInfo, ret.paramdesc, func))
                    {
                        ...
                        {
                            returnArgId = i;
                            retType = new TypeConverter(info.ConverterInfo, info.RefTypeInfo, ret.tdesc, isNewEnumMember, ConversionType.ParamRetVal);
        ...
        static public TypeConverter[] GenerateParameterTypes(InterfaceInfo info, InterfaceMemberInfo memInfo, FuncDesc func, bool isNewEnumMember, int vararg, out int lcidArg, out bool isStandardOleCall, out TypeConverter retTypeConverter, out ReturnKind kind, out int retArgId)
        {
                ...
                if (func.GetElemDesc(i).paramdesc.IsLCID)
                {
                    if (lcidArg >= 0)
                    {
                        info.ConverterInfo.ReportEvent(
                            ...
                            Resource.FormatString("Wrn_MultipleLcids", info.RefTypeInfo.GetDocumentation(), info.RefTypeInfo.GetDocumentation(func.memid)));
                    }
            ...
            for (int n = 0; n < cParams; ++n)
            {
                ...
                try                
                {
                    ...
                    typeConverterList.Add(new TypeConverter(info.ConverterInfo, info.RefTypeInfo, elem.tdesc, paramDesc, false, conversionType));
                }
