...
namespace tlbimp2
{
    ...
    class InterfaceMemberInfo
    {
        ...
        public bool IsProperty
        {
            get
            {
                return m_invokeKind != INVOKEKIND.INVOKE_FUNC;
            }
        }
        ...
        public bool IsPropertyGet
        {
            get
            {
                return (m_invokeKind & INVOKEKIND.INVOKE_PROPERTYGET) != 0;
            }
        }
        ...
        public bool IsPropertyPut
        {
            get
            {
                return (m_invokeKind & INVOKEKIND.INVOKE_PROPERTYPUT) != 0;
            }
        }
        ...
        public bool IsPropertyPutRef
        {
            get
            {
                return (m_invokeKind & INVOKEKIND.INVOKE_PROPERTYPUTREF) != 0;
            }
        }
        ...
        public static List<InterfaceMemberInfo> BuildMemberList(ConverterInfo info, TypeInfo type, IConvBase convBase, bool implementsIEnumerable)
        {
            ...
            {
                Dictionary<string, INVOKEKIND> propertyInvokeKinds = new Dictionary<string, INVOKEKIND>();
                ...
                for (int i = ConvCommon.GetIndexOfFirstMethod(type, attr); i < attr.cFuncs; ++i)
                {
                    using (FuncDesc funcDesc = type.GetFuncDesc(i))
                    {
                        ...
                        if (invKind == INVOKEKIND.INVOKE_FUNC) continue;

                        string name = type.GetDocumentation(funcDesc.memid);
                        if (!propertyInvokeKinds.ContainsKey(name))
                            propertyInvokeKinds.Add(name, (INVOKEKIND)0);

                        INVOKEKIND totalInvokeKind = propertyInvokeKinds[name];
                ...
                for (int i = 0; i < attr.cVars; ++i)
                {
                    ...
                    InterfaceMemberInfo memberInfo =
                        new InterfaceMemberInfo(info, type, i, name, getFuncName, InterfaceMemberType.Variable, INVOKEKIND.INVOKE_PROPERTYGET, memid, null, varDesc);
                ...
                foreach (InterfacePropertyInfo funcPropInfo in propertyList.Values)
                {
                    if (!funcPropInfo.DeterminePropType(info, convBase))
                    {
                        ...
                        if (funcPropInfo.Get != null)
                        {
                            ...
                            memInfo.m_invokeKind = INVOKEKIND.INVOKE_FUNC;
    ...
    class ConvInterfaceLocal : ConvLocalBase, IConvInterface
    {
        ...
        private TYPEFLAGS HandleDualIntf()
        {
            ...
            {
                TYPEFLAGS typeFlags = typeAttr.wTypeFlags;
