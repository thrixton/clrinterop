    ...
    public String Lookup(String strOpt, out bool bRequiresValue, out bool bCanHaveValue)
    {
        ...
        for (i = 0; i < m_aOptions.Length; i++)
        {
            ...
            if (m_aOptions[i].StartsWith(strOptLower))
            {
                ...
                if (bMatched)
                    throw new TlbImpGeneralException(Resource.FormatString("Err_AmbigousOption", strOpt), ErrorCode.Err_AmbigousOption, true);
        ...
        throw new TlbImpGeneralException(Resource.FormatString("Err_UnknownOption", strOpt), ErrorCode.Err_UnknownOption, true);
    }
}
